...
void inv_shift_rows(unsigned char *msg)
{
    ...
    tmp2 = msg[10]; tmp = msg[14];
    msg[14] = msg[6]; msg[10] = msg[2]; msg[6] = tmp; msg[2] = tmp2;
...
{
    bool left_most_bit = c & 1 << 7; //1을 왼쪽으로 7만큼 이동(=10000000) -> c랑 and연산(최상위 비트 판별)
	c = c << 1;
	if(left_most_bit) // 0: f, 1: t
		c = c ^ 27;
	return c;
}
...
void mix_columns(unsigned char *msg)
{
	...
	for(int y=0; y<4; y++)//mix의 행
	{ 
		for(int x=0; x<4; x++) //msg의 열
		{
			for(int i=0; i<4; i++) //각 행,열의 i번째
			{
				d = msg[4*x + i];
				switch (mix[y][i])
				{
			...
			result[4*x+y] = c[0] ^ c[1] ^ c[2] ^ c[3]; //배열 가로로 저장(0,4,8,12->1,5,9,13)
		}
	}
...
void key_schedule(unsigned char *key, unsigned char round_key[ROUND+1][16])
{
	...
	memcpy(round_key[0], key, 16); //round_key[0]에 key 대입
	...
	{	
		...
		*(p+3) = sbox[*(p+3)];
		...
		//xorRcons
		*p ^= rcon[i]; //현재 p에 저장된 값은 g

		//2. w4 = g ^ w0
		for(int j=0; j<N; j++, p++){
			*p ^= *(p-4*N); //*p = *p ^ *(p-4*N) = g ^ 바로 윗줄(w0)을 바이트 단위로
		}//for문이 끝나면 p에 저장된 값은 w5의 첫번째 바이트

		//3. w5, w6, w7
		for(int j=0; j<3*N; j++, p++){
			*p = *(p-4) ^ *(p-4*N); //내 바로 앞 워드에서 나랑 같은 위치의 값 ^ 내 바로 윗줄
		}
